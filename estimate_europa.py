...
estimation.py script. This model simulates 3D cartesian position observables every 2 days on Europa
with accuracy that is the same as the a priori uncertainty. After the stabilized a priori covariance is calculated it is possible to 
estimate the initial state of Europa by simulating the stellar occultation and propagate the covariance matrix.
"""
#%%
...
# Create tabulated settings fo Europa
...
acceleration_settings = {"Europa": acceleration_settings_europa}
...
link_ends_stellar[observation.transmitter] = ("Europa", "")
...
covariance_a_priori = np.block([
    [covariance_position_initial_eu, np.zeros((3,3))],
    [np.zeros((3,3)),covariance_velocity_initial_eu]
])
...
observation_settings_list_pos = observation.cartesian_position(link_ends_europa)
...
# Create the estimation object for Europa
...
# Simulate required observation on Europa
...
# Setup the weight matrix W with weights for Europa
...
plt.axvline(x = observation_times_europa/31536000,label = 'occ',color="magenta")
plt.ylim([10e2,10e4])
plt.yscale("log")
...
#%%
""""
Propagate RA and DEC of Europa
"""
da_dr = dict()
dd_dr = dict()
Ta_dict = dict()
