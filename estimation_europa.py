...
calendar_start = datetime.datetime(2020,1,1)
...
# Create tabulated settings fo Io Europa and Jupiter
...
bodies_to_propagate = ["Io","Jupiter","Europa"]
...
acceleration_settings = {"Io": acceleration_settings_io, "Jupiter": acceleration_settings_jup, "Europa": acceleration_settings_europa}
...
"""
The initial state of Io Europa and Jupiter that will be propagated is now defined. 
"""
...
"""
Propagate the dynamics of Jupiter Europa and Io and extract state transition and sensitivity matrices
"""
...
#15km RSW position 0.15,1.15,0.75m/s RSW velocity
rotation_rsw_to_inertial_dict_europa = dict()
for epoch in list(variational_equations_solver.state_history):
    rotation_rsw_to_inertial_dict_europa[epoch] = frame_conversion.rsw_to_inertial_rotation_matrix(states[epoch][12:]).reshape(3,3)
uncertainties_rsw_europa = np.zeros((3,3))
...
covariance_a_priori = np.block([
    [covariance_position_initial_io, np.zeros((3,3)), np.zeros((3,3)),np.zeros((3,3)), np.zeros((3,3)),np.zeros((3,3))],
    [np.zeros((3,3)),covariance_velocity_initial_io,np.zeros((3,3)), np.zeros((3,3)), np.zeros((3,3)),np.zeros((3,3))],
    [np.zeros((3,3)),np.zeros((3,3)), covariance_position_initial_jup, np.zeros((3,3)), np.zeros((3,3)),np.zeros((3,3))],
    [np.zeros((3,3)),np.zeros((3,3)), np.zeros((3,3)), covariance_velocity_initial_jup, np.zeros((3,3)),np.zeros((3,3))],
    [np.zeros((3,3)),np.zeros((3,3)), np.zeros((3,3)), np.zeros((3,3)), covariance_position_initial_europa,np.zeros((3,3))],
    [np.zeros((3,3)),np.zeros((3,3)), np.zeros((3,3)), np.zeros((3,3)),np.zeros((3,3)), covariance_velocity_initial_europa]
])
...
link_ends_stellar_io = dict()
link_ends_stellar_io[observation.receiver] = ("Earth", "")
link_ends_stellar_io[observation.transmitter] = ("Io", "")
link_ends_vlbi = dict()
link_ends_vlbi[observation.receiver] = ("Earth", "")
link_ends_vlbi [observation.transmitter] = ("Jupiter", "")
link_ends_stellar_europa = dict()
link_ends_stellar_europa[observation.receiver] = ("Earth", "")
link_ends_stellar_europa[observation.transmitter] = ("Europa", "")

# Create Bias settings 0.5 nrad in both RA and Dec


# Create observation settings for each link/observable
observation_settings_list_io = observation.angular_position(link_ends_stellar_io)
observation_settings_list_europa = observation.angular_position(link_ends_stellar_europa)
...
observation_simulation_settings_io = observation.tabulated_simulation_settings(
    ...
    link_ends_stellar_io,
    observation_times_io
)

# Define the observations for Europa
stellar_occ2 = datetime.datetime(2020,6,22)
stellar_occ2 = time_conversion.calendar_date_to_julian_day_since_epoch(stellar_occ2)*constants.JULIAN_DAY
observation_times_europa = np.array([stellar_occ2])

observation_simulation_settings_europa = observation.tabulated_simulation_settings(
    observation.angular_position_type,
    link_ends_stellar_europa,
...
# Add noise levels of roughly 1 mas to Io
...
# Add noise levels of roughly 1 mas to Europa
noise_level_europa = 4.85e-9
observation.add_gaussian_noise_to_settings(
...
pod_input.set_constant_weight_for_observable_and_link_ends(observation.angular_position_type,link_ends_stellar_io,weights_stellar)
pod_input.set_constant_weight_for_observable_and_link_ends(observation.angular_position_type,link_ends_stellar_europa,weights_stellar)
...
for epoch in list(variational_equations_solver.state_history):
    ...
    propagated_covariance_rsw_dict_europa[epoch] = lalg.multi_dot([rotation_rsw_to_inertial_dict_europa[epoch].T,propagated_covariance_dict[epoch][12:15,12:15],rotation_rsw_to_inertial_dict_europa[epoch]])
    propagated_formal_errors_rsw_dict_europa[epoch] = np.sqrt(np.diag(propagated_covariance_rsw_dict_europa[epoch]))
...
"""
t_io = time_io/31536000
t_eu = time_europa/31536000
t_ju = time_jup/31536000

plt.figure(figsize=(9,5))
plt.plot(t_io,values_io[:,0], label = 'R')
plt.plot(t_io,values_io[:,1], label = 'S')
plt.plot(t_io,values_io[:,2], label = 'W')
...
plt.figure(figsize=(9,5))
plt.plot(t_eu,values_europa[:,0], label = 'R')
plt.plot(t_eu,values_europa[:,1], label = 'S')
plt.plot(t_eu,values_europa[:,2], label = 'W')
plt.plot(observation_times_io/31536000, 100,'o')
plt.plot(observation_times_europa/31536000, 100,'o')
plt.yscale("log")
plt.grid(True, which="both", ls="-")
...
plt.figure(figsize=(9,5))
plt.plot(t_ju,values_jup[:,0], label = 'R')
plt.plot(t_ju,values_jup[:,1], label = 'S')
plt.plot(t_ju,values_jup[:,2], label = 'W')
plt.yscale("log")
plt.grid(True, which="both", ls="-")
...
initial_state = np.savetxt("/Users/gianmarcobroilo/Desktop/ThesisResults/Io+Europa/initial_state.dat",state_array)
uncertainty_io = np.savetxt("/Users/gianmarcobroilo/Desktop/ThesisResults/Io+Europa/uncertainty_io.dat",values_io)
uncertainty_europa = np.savetxt("/Users/gianmarcobroilo/Desktop/ThesisResults/Io+Europa/uncertainty_europa.dat",values_europa)
uncertainty_jup = np.savetxt("/Users/gianmarcobroilo/Desktop/ThesisResults/Io+Europa/uncertainty_jup.dat",values_jup)
time_prop = np.savetxt("/Users/gianmarcobroilo/Desktop/ThesisResults/Io+Europa/time_prop.dat",time_io)
obs2 = np.savetxt("/Users/gianmarcobroilo/Desktop/ThesisResults/Io+Europa/observations_vlbi.dat",observation_times_jup)